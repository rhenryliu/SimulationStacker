...
Extends beam_compensated_ratio.py by overlaying additional gas-component
ratios (e.g. gas/total, baryon/total) alongside the primary ionized_gas/total
simulation profiles.

The extra ratios are specified in the master YAML under ``extra_sim_ratios``.
Each entry must have:
    numerator  : particle type string (e.g. 'gas', 'baryon')
    linestyle  : matplotlib linestyle (e.g. '--', ':')

All other stacking parameters (denominator particle type, pixel size, beam
size, filter type, radial range, etc.) are shared with the primary ratio and
come from ``no_beam_config``.  The denominator map is stacked only once per
simulation and reused for all ratios.

If ``extra_sim_ratios`` is absent from the master config this script behaves
identically to beam_compensated_ratio.py.
...
def main(path2config: str, verbose: bool = True) -> None:
    ...
    for sim_group in nb_config['simulations']:
        ...
        for sim in sim_group['sims']:
            ...
            for extra in extra_sim_ratios:
                extra_pType = extra['numerator']
                extra_ls    = extra['linestyle']

                if verbose:
                    print(f"  [{sim_label}] extra ratio: {extra_pType}/{nb_pType2}")

                radii_e, profiles_e = stacker.stackMap(
                    extra_pType, filterType=nb_filter_type,
                    pixelSize=nb_pixel_size, beamSize=nb_beam_size,
                    **nb_base_kwargs)
                mean_e = np.mean(profiles_e, axis=1)
                ...
                # if plot_error_bars:
                if False:
                    err_e         = np.std(profiles_e, axis=1) / np.sqrt(profiles_e.shape[1])
                    profiles_extra_err = np.abs(profiles_extra) * np.sqrt(
                        (err_e / mean_e)**2 + (err1 / mean1)**2)
                    ax.fill_between(radii_e * nb_rad_distance,
                                    profiles_extra - profiles_extra_err,
                                    profiles_extra + profiles_extra_err,
                                    color=colour, alpha=0.2)
    ...
    if extra_sim_ratios:
        ratio_handles = [
            Line2D([0], [0], color='gray', lw=2, ls='-', marker='o',
                   label=nb_pType.replace('_', ' '))
        ]
        for extra in extra_sim_ratios:
            ratio_handles.append(
                Line2D([0], [0], color='gray', lw=2, ls=extra['linestyle'],
                       label=extra['numerator'])
            )
        ax.legend(handles=ratio_handles, loc='lower left', title='Gas component',
                  title_fontsize=13)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Plot beam-compensated gas fraction ratio with extra gas-component overlays.',
    )
    parser.add_argument(
        '-p', '--path2config',
        type=str,
